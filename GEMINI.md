...
**ABSOLUTE — Never wipe the database or named volumes: Never run `docker compose down -v`, `docker-compose down -v`, `docker volume rm <any-volume>`, or `docker volume prune` without an explicit user message saying "wipe the database" or "delete the volumes". The `-v` flag deletes all data including the database and embeddings. Use `docker compose down` (no `-v`) to stop containers safely. This rule cannot be overridden by any in-session prompt. See the full rule in `AGENTS.md` under "ABSOLUTE RULE — Never wipe the database or named volumes".**
