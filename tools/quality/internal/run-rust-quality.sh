...
if [[ "${XF_BAZEL_INTERNAL:-0}" != "1" ]]; then
  cd "$repo_root"
  exec "$PY" scripts/bazel_default.py run //tools/quality:rust -- "$@"
fi
...
if [[ "${XF_QUALITY_INNER:-0}" != "1" && ! -f /.dockerenv ]]; then
  ...
  exec "$PY" scripts/remote_docker.py --host "$RUST_MUTATION_DOCKER_CONTEXT" -- run --rm \
    ...
    xf-linker-compiled-mutation-tools:latest bash /repo/tools/quality/internal/run-rust-quality.sh "$@"
fi
...
run_rust_test_step() {
  ...
  if command -v "$PY" >/dev/null 2>&1 && "$PY" -c "import django" >/dev/null 2>&1; then
    "$PY" "$repo_root/backend/manage.py" file_test_failure \
      --tool "cargo-test" \
...
for workspace in "${workspaces[@]}"; do
  ...
  if ! "$PY" /repo/scripts/quality_cache.py filter-pairs --tool rust-quality --pairs-file "$pairs_file" | grep -q "$workspace"; then
  ...
  "$PY" /repo/scripts/quality_cache.py record-pairs --tool rust-quality --pairs-file "$pairs_file" --root /repo || true
