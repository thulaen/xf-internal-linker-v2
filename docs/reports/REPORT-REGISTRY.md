...
- **Status:** OPEN (3 of 5 findings unresolved)
...
- **Status:** OPEN
- **Proposed fix:** Add a custom Channels middleware that reads `?token=<value>` from the handshake URL, looks up the `Token` row, and sets `scope["user"]`. Update the two services to append the token as a query param when opening the socket. Keep `AuthMiddlewareStack` underneath as a fallback for anything that still uses cookies. Alternative: add a `SessionTokenMiddleware` that issues a short-lived cookie on `POST /api/auth/token/` so the existing `AuthMiddlewareStack` sees it.
- **Regression watch:** Any change to `backend/config/asgi.py` must preserve an auth path that understands token-auth. When fixing, verify the dev-mode `environment.ts` WS URL still works (either keep direct-to-Daphne and add the token middleware to the plain consumer, or change `wsBaseUrl` to proxy through nginx at port 80 and run the middleware on that path).
