# OPT-86 -- Session Co-occurrence Matrix Builder

## Overview
**Category:** Analytics acceleration
**Extension file:** `cooccur_matrix.cpp` (NEW)
**Expected speedup:** >=3x over Python O(N^2) loop in cooccurrence/services.py
**RAM:** <20 MB | **Disk:** <1 MB
**Research basis:** Sarwar B. et al., "Item-based collaborative filtering recommendation algorithms", WWW 2001. Session co-occurrence with Jaccard similarity.

## Algorithm

Input: list of session paths (each session = set of page IDs). For all session pairs sharing >= min_co_session_count pages, compute Jaccard similarity. Output: sparse triplets (page_a, page_b, jaccard_score). Uses sorted page-ID arrays for O(n) merge-intersection. TBB parallel_for over session pairs. Only emits pairs above min_jaccard threshold.

## C++ Interface (pybind11)

```cpp
// cooccur_matrix.cpp
// py::list build_cooccurrence_triplets(
//     const std::vector<std::vector<uint32_t>>& session_paths,
//     uint32_t min_co_sessions,
//     float min_jaccard);
//
// Returns list of (page_a, page_b, score) tuples.
```

## Memory Budget
- Runtime RAM: <20 MB for 10K sessions (sparse output only)
- Disk: <1 MB

## Performance Target
- Target: >=3x faster than Python O(N^2) loop
- Benchmark: 10K sessions, 1000 iterations
...
## Dependencies
- TBB (Linux) or std::execution::par (Windows) for parallelism

## Test Plan
- Output matches Python reference within 1e-4
- Edge cases: empty sessions, single session, two identical sessions, no pairs above threshold
