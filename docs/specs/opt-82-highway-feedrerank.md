# OPT-82 -- Highway SIMD for feedrerank

## Overview
**Category:** Google library -- SIMD
**Extension file:** `feedrerank.cpp` (modify existing)
**Expected speedup:** >=3x MMR cosine similarity loop
...
Replace the cosine similarity inner loop in calculate_mmr_scores_batch() with Highway SIMD. The MMR algorithm computes max_cosine_sim between each candidate and all selected embeddings -- this is the hot path (O(candidates * selected * dimensions)). Highway explicit SIMD for the dot-product-per-pair computation.

## C++ Interface (pybind11)

```cpp
// No API change:
// calculate_mmr_scores_batch() signature unchanged.
// HWY_DYNAMIC_DISPATCH for runtime ISA selection.
```

## Memory Budget
- Runtime RAM: 0 additional
- Disk: ~3 MB shared

## Performance Target
- Target: >=3x faster MMR scoring
- Benchmark: 500 candidates, 50 selected, 1024D embeddings, 100 iterations
...
## Dependencies
- Highway (shared with OPT-80, 81)
- feedrerank already uses TBB for row-level parallelism -- Highway handles the inner dot-product loop

## Test Plan
- Correctness: MMR scores and max_similarities match current output within 1e-4
- Edge cases: zero selected embeddings, single candidate, identical embeddings
