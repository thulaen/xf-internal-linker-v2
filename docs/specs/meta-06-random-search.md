# META-06-RANDOM-SEARCH — Random Search Weight Sampler

## Overview
**Category:** Weight optimizer
**Extension file:** `random_search.cpp`
**Replaces/improves:** Grid search in Python
**Expected speedup:** ≥10x over Python random loop
**RAM:** <2 MB | **Disk:** <1 MB

## Algorithm

Draw wᵢ ~ LogUniform(0.01, 10) for T=500 iterations. Keep w* = argmax NDCG(w) on held-out fold. O(T × eval_cost).

## C++ Interface (pybind11)

```cpp
std::vector<float> random_search(int d, int T, uint64_t seed, const float* scores_matrix, const int* labels, int n_q, int max_c);
```
...
## Performance Target
- Target: ≥10x over Python random loop
...
- T=1: returns first sample
- All-zero NDCG: returns initial weights unchanged
