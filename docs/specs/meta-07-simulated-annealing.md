# META-07-SIMULATED-ANNEALING — Simulated Annealing Ranker

## Overview
**Category:** Weight optimizer
**Extension file:** `sim_anneal.cpp`
**Replaces/improves:** Manual weight search
**Expected speedup:** ≥3x over Python loop
**RAM:** <2 MB | **Disk:** <1 MB

## Algorithm

Perturb: w' = w + N(0, T × I). Accept if NDCG(w') > NDCG(w) or with prob exp((NDCG(w')-NDCG(w))/T). T ← 0.99×T each step. O(max_steps × eval_cost).

## C++ Interface (pybind11)

```cpp
std::vector<float> simulated_annealing(const float* init, int d, int max_steps, float T_init, float cooling);
```
...
## Test Plan
- Monotonic improvement when T=0 (greedy mode)
- High T: accepts worse solutions (verified statistically)
