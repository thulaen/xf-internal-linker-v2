# OPT-79 -- Abseil FixedArray

## Overview
**Category:** Google library -- stack array
**Extension file:** `scoring.cpp`, `simsearch.cpp` (modify existing)
**Expected speedup:** >=3x for temporary score buffer allocation
**RAM:** Net negative (uses stack instead of heap) | **Disk:** ~5 MB shared Abseil install
**Research basis:** Abseil documentation. absl::FixedArray allocates on the stack when size <= inline_elements, falls back to heap otherwise. Eliminates malloc/free for known-small temporary arrays.

## Algorithm

Replace `std::vector<float>` temporary score arrays in calculate_composite_scores_full_batch() and score_and_topk() with `absl::FixedArray<float>`. Typical batch sizes (100-1000 candidates) fit comfortably in stack allocation. Avoids heap round-trip for every batch call.

## C++ Interface (pybind11)

```cpp
// No API change -- internal replacement:
// absl::FixedArray<float> scores(n);  // replaces std::vector<float> scores(n);
// Stack allocation for n <= 8192, heap fallback for larger.
```

## Memory Budget
- Runtime RAM: Net negative (avoids heap metadata for common case)
- Disk: ~5 MB shared

## Performance Target
- Target: >=3x fewer heap allocations per pipeline run
- Benchmark: 1000 batch calls with 500 candidates each
...
- Edge cases: n=0, n=1, n=8192 (boundary), n=10000 (heap fallback)
