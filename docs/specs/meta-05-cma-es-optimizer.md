# META-05-CMA-ES-OPTIMIZER — CMA-ES Weight Optimizer

## Overview
**Category:** Weight optimizer
**Extension file:** `cma_es.cpp`
**Replaces/improves:** Manual weight tuning
**Expected speedup:** ≥3x over Python CMA-ES
**RAM:** <10 MB | **Disk:** <1 MB

## Algorithm

Sample λ=20 weight vectors from N(μ, C). Evaluate NDCG per vector. Update μ ← weighted mean of top-μ. Update C via rank-1 + rank-μ covariance update. O(d² × λ) per generation.

## C++ Interface (pybind11)

```cpp
std::vector<float> cma_es_optimize(const float* init_weights, int d, int n_generations, int lambda_pop);
```
...
## Test Plan
- Sphere function: converges to origin within 100 generations
- NaN input: raises ValueError
- d=1: degenerates to line search correctly
