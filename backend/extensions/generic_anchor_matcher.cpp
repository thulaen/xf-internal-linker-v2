...
inline std::size_t goto_index(std::int32_t node, std::uint8_t byte) {
    return (static_cast<std::size_t>(node) << 8U) | static_cast<std::size_t>(byte);
}

// Insert *phrase* into the trie portion of *aut*; record the phrase
// index in the output list of the terminal node.
void insert_phrase(Automaton& aut, std::int32_t phrase_idx, const std::string& phrase) {
...
void build_fail_links(Automaton& aut) {
    ...
    while (!q.empty()) {
        ...
        for (std::uint16_t b = 0; b < 256; ++b) {
            ...
            // termination).
            while (f != 0 &&
                   aut.goto_table[goto_index(f, static_cast<std::uint8_t>(b))] == NO_TRANSITION) {
            ...
            aut.fail_link[static_cast<std::size_t>(child)] = (link == child) ? 0 : link;
...
std::vector<std::string> find_all(std::shared_ptr<Automaton> aut, const std::string& text) {
    ...
    for (char c : text) {
        ...
        while (node != 0 && aut->goto_table[goto_index(node, byte)] == NO_TRANSITION) {
        ...
        for (std::int32_t phrase_idx : aut->output[static_cast<std::size_t>(node)]) {
