...
double personalized_pagerank_step_core(const int32_t* indptr_ptr, const int32_t* indices_ptr,
                                       const double* data_ptr, const double* ranks_ptr,
                                       const bool* dangling_ptr, const double* personalization_ptr,
                                       double damping, int node_count, double* next_ptr) {
...
void hits_step_core(const int32_t* indptr_ptr, const int32_t* indices_ptr, const double* data_ptr,
                    const double* authority_ptr, const double* hub_ptr, int node_count,
...
std::tuple<py::array_t<double>, double> personalized_pagerank_step(
    ...
    py::array_t<double, py::array::c_style | py::array::forcecast> personalization, double damping,
    int node_count) {
    auto indptr_buf = indptr.request();
    auto indices_buf = indices.request();
    auto data_buf = data.request();
    ...
    if (indptr_buf.ndim != 1 || indices_buf.ndim != 1 || data_buf.ndim != 1 ||
        ranks_buf.ndim != 1 || dangling_buf.ndim != 1 || personalization_buf.ndim != 1) {
        throw std::runtime_error("personalized_pagerank_step expects 1D CSR arrays and vectors");
    }
    if (node_count < 0) {
        throw std::runtime_error("node_count must be non-negative");
    }
    ...
    if (static_cast<size_t>(personalization_buf.shape[0]) != static_cast<size_t>(node_count)) {
    ...
    double delta;
    {
        ...
        delta = personalized_pagerank_step_core(
            ...
            static_cast<const int32_t*>(indices_buf.ptr), static_cast<const double*>(data_buf.ptr),
            static_cast<const double*>(ranks_buf.ptr), static_cast<const bool*>(dangling_buf.ptr),
            static_cast<const double*>(personalization_buf.ptr), damping, node_count,
...
std::tuple<py::array_t<double>, py::array_t<double>> hits_step(
    ...
    py::array_t<double, py::array::c_style | py::array::forcecast> hub, int node_count) {
    auto indptr_buf = indptr.request();
    auto indices_buf = indices.request();
    auto data_buf = data.request();
    ...
    if (static_cast<size_t>(authority_buf.shape[0]) != static_cast<size_t>(node_count)) {
        throw std::runtime_error("authority length must equal node_count");
    }
    if (static_cast<size_t>(hub_buf.shape[0]) != static_cast<size_t>(node_count)) {
...

    {
        ...
        hits_step_core(
            static_cast<const int32_t*>(indptr_buf.ptr),
            static_cast<const int32_t*>(indices_buf.ptr), static_cast<const double*>(data_buf.ptr),
            static_cast<const double*>(authority_buf.ptr), static_cast<const double*>(hub_buf.ptr),
            node_count, static_cast<double*>(next_authority_buf.ptr),
...
PYBIND11_MODULE(pagerank, m) {
    ...
    m.def("personalized_pagerank_step", &personalized_pagerank_step,
