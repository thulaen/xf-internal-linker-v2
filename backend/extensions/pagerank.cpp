...
std::tuple<py::array_t<double>, double> personalized_pagerank_step(
    ...
    py::array_t<double, py::array::c_style | py::array::forcecast> personalization,
    ...
    double delta;
    {
        ...
        delta = personalized_pagerank_step_core(
            ...
            static_cast<const double*>(personalization_buf.ptr),
            damping, node_count, static_cast<double*>(next_buf.ptr));
    }
...
std::tuple<py::array_t<double>, py::array_t<double>> hits_step(
...

    {
        ...
        hits_step_core(
            ...
            static_cast<const double*>(hub_buf.ptr),
            node_count,
...
PYBIND11_MODULE(pagerank, m) {
    ...
    m.def("hits_step", &hits_step,
          "Run one Kleinberg HITS iteration step (authority + hub) from forward CSR.");
}
#endif
