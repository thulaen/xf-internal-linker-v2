...
void mmr_scores_core(const double* relevance, size_t candidate_count, const double* candidate_ptr,
                     const double* selected_ptr, size_t selected_count, size_t embedding_width,
                     double diversity_lambda, double* mmr_ptr, double* max_sim_ptr) {
...
py::array_t<double> calculate_rerank_factors_batch(
    ...
    if (observation_confidences_buf.ndim != 1 ||
        ...
        throw std::runtime_error(
            "observation_confidences must be a 1D float64 "
...
py::tuple calculate_mmr_scores_batch(
    ...
    if (candidate_buf.shape[1] != selected_buf.shape[1]) {
        throw std::runtime_error(
            "candidate_embeddings and selected_embeddings "
    ...
    {
        py::gil_scoped_release release;
        mmr_scores_core(static_cast<const double*>(relevance_buf.ptr), candidate_count,
