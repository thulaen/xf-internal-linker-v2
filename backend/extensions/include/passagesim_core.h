...
extern "C" {
/**
 * Compute the maximum cosine similarity between a query vector and a matrix of passage vectors.
 * 
 * @param query_ptr Pointer to the query vector (float[dim])
 * @param matrix_ptr Pointer to the matrix of passage vectors (float[num_passages * dim])
...
void c_passagesim_maxsim(const float* query_ptr, const float* matrix_ptr, 
                         size_t num_passages, size_t dim, 
                         float* out_sim, size_t* out_index);
}
