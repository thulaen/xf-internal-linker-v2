...
static py::array_t<int32_t> ivf_search(
    ...
    const std::vector<std::vector<int32_t>>& partition_member_lists,
  ...
  if (top_k < 1) throw std::runtime_error("top_k must be >= 1");
  ...
  c_ivf_build_adc_lut(query_ptr, rotation_ptr, codebooks_ptr, dim, m, k,
                      lut.data());
  ...
  std::priority_queue<ScoredItem, std::vector<ScoredItem>, decltype(cmp)> heap(
      cmp);
...
static py::tuple adc_score_destination(
  ...
  const float cosine_sim =
      std::max(0.0f, std::min(1.0f, 1.0f - best_dist * 0.5f));
...
PYBIND11_MODULE(ivf_index, m) {
  ...
  m.def("ivf_search", &ivf_search,
        ...
        py::arg("query"), py::arg("centroids"),
        py::arg("partition_member_lists"), py::arg("opq_codes"),
  ...
  m.def("adc_score_destination", &adc_score_destination,
        ...
        py::arg("query"), py::arg("opq_codes"), py::arg("rotation"),
        py::arg("codebooks"));
}
