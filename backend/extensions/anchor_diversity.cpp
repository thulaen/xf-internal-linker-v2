...
void evaluate_anchor_diversity_core(
    ...
    const int32_t* exact_match_counts_before,
    std::size_t count,
    int32_t min_history_count,
    double max_exact_match_share,
    int32_t max_exact_match_count,
    bool hard_cap_enabled,
    int32_t* out_projected_exact_count,
    double* out_projected_exact_share,
    double* out_share_overflow,
    double* out_count_overflow_norm,
    double* out_spam_risk,
    double* out_score_anchor_diversity,
    int32_t* out_state_index,
    uint8_t* out_would_block) {
    for (std::size_t i = 0; i < count; ++i) {
        ...
        // PARITY: matches anchor_diversity.py line 137 — projected_exact_match_count
        const int32_t projected_count = before + 1;

        // PARITY: matches anchor_diversity.py lines 138-141 — projected_exact_share
        const int32_t denom_count = std::max<int32_t>(active + 1, MIN_COUNT_DENOMINATOR);
        const double projected_share = static_cast<double>(projected_count) /
                                       static_cast<double>(denom_count);
        ...
        const double share_overflow =
            std::max(0.0, projected_share - max_exact_match_share) / share_denominator;
        ...
        const int32_t count_denom = std::max<int32_t>(max_exact_match_count, MIN_COUNT_DENOMINATOR);
        const double count_overflow_norm = std::min(
            SPAM_RISK_CEILING,
            static_cast<double>(count_overflow_abs) / static_cast<double>(count_denom));

        // PARITY: matches anchor_diversity.py line 154 — spam_risk
        const double spam_risk = std::min(
            SPAM_RISK_CEILING,
            (SHARE_WEIGHT * share_overflow) + (COUNT_WEIGHT * count_overflow_norm));
        ...
        const bool blocked = hard_cap_enabled && (projected_count > max_exact_match_count);
...
py::dict evaluate_batch(
    py::array_t<int32_t, py::array::c_style | py::array::forcecast> active_anchor_counts,
    py::array_t<int32_t, py::array::c_style | py::array::forcecast> exact_match_counts_before,
    int32_t min_history_count,
    double max_exact_match_share,
    int32_t max_exact_match_count,
    bool hard_cap_enabled) {
    ...
    if (active_buf.ndim != 1 || before_buf.ndim != 1) {
        throw std::runtime_error(
            "active_anchor_counts and exact_match_counts_before must be 1-D int32 arrays");
    }
...

    {
        ...
        evaluate_anchor_diversity_core(
            ...
            static_cast<const int32_t*>(before_buf.ptr),
            count,
            min_history_count,
...
PYBIND11_MODULE(anchor_diversity, m) {
    ...
    m.def("evaluate_batch",
          &evaluate_batch,
          py::arg("active_anchor_counts"),
          py::arg("exact_match_counts_before"),
          py::arg("min_history_count"),
          py::arg("max_exact_match_share"),
          py::arg("max_exact_match_count"),
