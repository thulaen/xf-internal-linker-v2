...
// calls (GSC, GA4, Matomo, XenForo, WordPress). Spec:
...
class RateLimiterRegistry {
 ...
 private:
  ...
  std::uint64_t next_midnight_ns_(std::uint64_t now_mono_ns) const {
    ...
    std::tm utc{};
#if defined(_WIN32)
    if (gmtime_s(&utc, &wall_t) != 0) {
      throw std::runtime_error("failed to calculate UTC reset time");
    }
#else
    if (gmtime_r(&wall_t, &utc) == nullptr) {
      throw std::runtime_error("failed to calculate UTC reset time");
    }
#endif
...
PYBIND11_MODULE(api_rate_limiter, m) {
  ...
  py::class_<RateLimiterRegistry>(m, "RateLimiterRegistry")
      ...
      .def("register_bucket", &RateLimiterRegistry::register_bucket,
           py::arg("name"), py::arg("capacity"), py::arg("rate_per_sec"),
           py::arg("daily_quota") = -1,
