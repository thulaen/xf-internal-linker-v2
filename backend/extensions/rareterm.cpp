...
evaluate_rare_terms_core(const std::vector<std::string> &terms,
  ...
  if (term_evidences.size() != term_count ||
      supporting_pages.size() != term_count) {
    throw std::runtime_error("terms, term_evidences, and supporting_pages must "
  ...
  for (size_t index = 0; index < term_count; ++index) {
    ...
    matched_terms.push_back(
        {terms[index], term_evidences[index], supporting_pages[index]});
  }
  ...
  const size_t keep_count =
      std::min(static_cast<size_t>(max_terms), matched_terms.size());
...
#ifndef XF_BENCH_MODE
std::pair<bool, double>
evaluate_rare_terms(const std::vector<std::string> &terms,
                    const std::vector<double> &term_evidences,
                    const std::vector<int> &supporting_pages,
  ...
  return evaluate_rare_terms_core(terms, term_evidences, supporting_pages,
                                  host_token_set, max_terms);
}
