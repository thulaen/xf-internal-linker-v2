...
py::array_t<float> score_full_batch(py::array_t<float> component_scores,
  ...
  cscore_full_batch(static_cast<const float*>(c_info.ptr), num_rows,
                    num_components, static_cast<const float*>(w_info.ptr),
                    num_weights, static_cast<const float*>(s_info.ptr),
                    num_silo, static_cast<float*>(res_info.ptr));

  return result;
}

PYBIND11_MODULE(scoring, m) {
  m.def("score_full_batch", &score_full_batch,
        "Compute final scores for a batch");
}
#endif
