...
std::vector<float> calculate_composite_scores(
    const std::vector<Candidate> &candidates, float w_semantic, float w_keyword,
    float w_node, float w_quality, float w_pr, float w_freshness, float w_ga4) {
  ...
  std::for_each(
      std::execution::par, indices.begin(), indices.end(), [&](size_t i) {
        ...
        results[i] = (c.score_semantic * w_semantic) +
                     (c.score_keyword * w_keyword) + (c.score_node * w_node) +
                     (c.score_quality * w_quality) + (c.score_pr * w_pr) +
...
py::array_t<float> calculate_composite_scores_full_batch(
  ...
  if (static_cast<size_t>(weight_buf.shape[0]) != k_components) {
    throw std::runtime_error(
        "weights length must match component_scores.shape[1]");
  }
  if (static_cast<size_t>(silo_buf.shape[0]) != n_rows) {
    throw std::runtime_error(
        "silo length must match component_scores.shape[0]");
  }
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, n_rows),
                      [&](const tbb::blocked_range<size_t> &r) {
                        for (size_t row = r.begin(); row < r.end(); ++row) {
                          ...
                          for (size_t col = 0; col < k_components; ++col) {
                            total +=
                                component_ptr[offset + col] * weight_ptr[col];
                          }
                          result_ptr[row] = total;
                        }
                      });
...
PYBIND11_MODULE(scoring, m) {
  ...
  m.def("calculate_composite_scores_full_batch",
