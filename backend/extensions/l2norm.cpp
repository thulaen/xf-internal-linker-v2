...
void normalize_l2(
  ...
  if (buf.size == 0)
    return;
  l2norm_normalize(static_cast<float *>(buf.ptr),
                   static_cast<size_t>(buf.size));
}

void normalize_l2_batch(
    py::array_t<float, py::array::c_style | py::array::forcecast> input) {
  auto buf = input.request();
  if (buf.ndim != 2)
    throw std::runtime_error("Input must be 2D");
  l2norm_normalize_batch(static_cast<float *>(buf.ptr),
                         static_cast<size_t>(buf.shape[0]),
                         static_cast<size_t>(buf.shape[1]));
}

PYBIND11_MODULE(l2norm, m) {
  m.def("normalize_l2", &normalize_l2,
        "In-place L2 normalization for 1D array");
