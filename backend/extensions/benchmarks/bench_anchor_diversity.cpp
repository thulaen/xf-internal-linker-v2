...
void BM_EvaluateAnchorDiversity(benchmark::State &state) {
  ...
  for (auto _ : state) {
    evaluate_anchor_diversity_core(
        ...
        /*hard_cap_enabled=*/false, out_projected_count.data(),
        out_projected_share.data(), out_share_overflow.data(),
        out_count_overflow_norm.data(), out_spam_risk.data(), out_score.data(),
        out_state_index.data(), out_would_block.data());
