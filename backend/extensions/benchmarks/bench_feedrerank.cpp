...
void BM_RerankFactors(benchmark::State &state) {
  ...
  for (auto _ : state) {
    rerank_factors_core(successes.data(), totals.data(),
                        observation_confidences.data(), count, 10000, 1.0, 1.0,
                        0.3, 0.1, out.data());
...
void BM_MmrScores(benchmark::State &state) {
  ...
  for (auto _ : state) {
    mmr_scores_core(relevance.data(), n_candidates, candidates.data(),
                    selected.data(), n_selected, dim, 0.7, out_mmr.data(),
                    out_sim.data());
