...
static void BM_QuantEmb_OPQ_Encode(benchmark::State& state) {
  ...
  state.SetItemsProcessed(state.iterations() *
                          static_cast<benchmark::IterationCount>(num_vectors));
}

// Benchmark across common batch sizes: 1, 8, 32, 64
BENCHMARK(BM_QuantEmb_OPQ_Encode)->Arg(1)->Arg(8)->Arg(32)->Arg(64);

static void BM_QuantEmb_OPQ_Train(benchmark::State& state) {
  size_t num_vectors = static_cast<size_t>(state.range(0));
  size_t dim = 128;
  size_t m = 16;
  size_t k = 16;
  ...
  std::vector<float> vectors(num_vectors * dim);
  std::vector<float> rotation(dim * dim);
  std::vector<float> codebooks(m * k * (dim / m));
  ...
  for (auto _ : state) {
    c_opq_train_identity_kmeans(vectors.data(), num_vectors, dim, m, k, n_iter,
...
BENCHMARK(BM_QuantEmb_OPQ_Train)->Arg(32)->Arg(128)->Arg(512);
