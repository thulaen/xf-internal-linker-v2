...
void test_single_row_single_component() {
  ...
  cscore_full_batch(components.data(), 1, 1, weights.data(), 1, silo.data(), 1,
                    out.data());
...
void test_zero_weights() {
  ...
  cscore_full_batch(components.data(), 1, 3, weights.data(), 3, silo.data(), 1,
                    out.data());
...
void test_mismatch_guard() {
  ...
  cscore_full_batch(components.data(), 1, 2, weights.data(), 1, silo.data(), 1,
                    out.data());

  /* Implementation returns without writing when sizes mismatch;
   * sentinel must be unchanged. */
  assert(std::fabs(out[0] - 99.0f) < kTol &&
         "mismatch must leave output unchanged");
...
void test_large_n() {
  ...
  std::vector<float> weights(k,
                             0.25f); /* weighted sum = k * 1.0 * 0.25 = 1.0 */
  std::vector<float> silo(n, 0.5f);  /* each out[i] = 0.5 + 1.0 = 1.5 */
  std::vector<float> out(n, 0.0f);

  cscore_full_batch(components.data(), n, k, weights.data(), k, silo.data(), n,
                    out.data());
