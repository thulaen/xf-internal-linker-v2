...
static void BM_IvfFindTopCentroids(benchmark::State& state) {
    ...
    for (auto _ : state) {
        c_ivf_find_top_centroids(query.data(), centroids.data(), n_centroids, kDim, nprobe,
...
static void BM_IvfBuildAdcLut(benchmark::State& state) {
    ...
    for (auto _ : state) {
        c_ivf_build_adc_lut(query.data(), rotation.data(), codebooks.data(), kDim, kSubquantisers,
                            kCentroidsPerSubq, lut.data());
...
static void BM_IvfAdcDistanceLoop(benchmark::State& state) {
    ...
    for (auto& x : codes)
        x = static_cast<uint8_t>(code_dis(gen));

    for (auto _ : state) {
        float total = 0.0f;
        for (size_t i = 0; i < pop; ++i) {
            total += c_ivf_adc_distance(&codes[i * kSubquantisers], lut.data(), kSubquantisers,
                                        kCentroidsPerSubq);
        }
