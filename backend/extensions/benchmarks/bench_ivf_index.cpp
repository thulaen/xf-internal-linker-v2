...
static void BM_IvfFindTopCentroids(benchmark::State& state) {
  ...
  for (auto _ : state) {
    c_ivf_find_top_centroids(query.data(), centroids.data(), n_centroids, kDim,
                             nprobe, out_ids.data(), out_dists.data());
    benchmark::DoNotOptimize(out_ids.data());
    benchmark::DoNotOptimize(out_dists.data());
  }
  state.SetItemsProcessed(state.iterations() *
                          static_cast<int64_t>(n_centroids));
}
...
static void BM_IvfBuildAdcLut(benchmark::State& state) {
  ...
  std::vector<float> codebooks(kSubquantisers * kCentroidsPerSubq *
                               (kDim / kSubquantisers));
  for (auto& x : query) x = dis(gen);
  ...
  for (auto _ : state) {
    c_ivf_build_adc_lut(query.data(), rotation.data(), codebooks.data(), kDim,
                        kSubquantisers, kCentroidsPerSubq, lut.data());
...
static void BM_IvfAdcDistanceLoop(benchmark::State& state) {
  ...
  std::uniform_int_distribution<int> code_dis(
      0, static_cast<int>(kCentroidsPerSubq) - 1);
  ...
  for (auto _ : state) {
    ...
    for (size_t i = 0; i < pop; ++i) {
      total += c_ivf_adc_distance(&codes[i * kSubquantisers], lut.data(),
