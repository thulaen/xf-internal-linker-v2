...
void BM_ScoreAndTopK(benchmark::State &state) {
  ...
  auto rows = xf_bench::random_int32s(
      n_candidates, 0, static_cast<int32_t>(n_candidates - 1), 44);
  ...
  for (auto _ : state) {
    cscore_and_topk(dest.data(), dim, sentences.data(), n_candidates, dim,
                    rows.data(), n_candidates, 50, out_indices.data(),
