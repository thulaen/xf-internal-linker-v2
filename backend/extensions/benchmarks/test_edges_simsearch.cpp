 ...
 *   out_indices[i] is a POSITION in the candidate array (0 ..
 * candidate_count-1), NOT a raw sentence-matrix row number. The caller maps
 * positions back to row numbers via candidate_rows[out_indices[i]].
 *
...
void test_empty_candidates() {
  ...
  cscore_and_topk(dest.data(), dim, sentences.data(), 1, dim,
                  ...
                  /* top_k */ 5, out_indices.data(), out_scores.data(),
                  &out_count);
...
void test_zero_topk() {
  ...
  cscore_and_topk(dest.data(), dim, sentences.data(), 1, dim, rows.data(), 1,
                  /* top_k */ 0, out_indices.data(), out_scores.data(),
                  &out_count);
...
void test_single_candidate() {
  ...
  assert(out_indices[0] == 0 &&
         "position in candidate array must be 0 for single candidate");
...
void test_topk_clamp() {
  ...
  cscore_and_topk(dest.data(), dim, sentences.data(), 2, dim, rows.data(), 2,
                  100, out_indices.data(), out_scores.data(), &out_count);
...
void test_nan_in_sentences() {
  ...
  std::vector<float> sentences = {std::numeric_limits<float>::quiet_NaN(),
                                  0.0f};
  std::vector<int32_t> rows = {0};
...
void test_large_n() {
  ...
  cscore_and_topk(dest.data(), dim, sentences.data(), n, dim, rows.data(), n,
                  50, out_indices.data(), out_scores.data(), &out_count);
