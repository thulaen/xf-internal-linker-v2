...
double score_field_tokens(const std::vector<std::string>& tokens, const std::vector<int>& host_tfs,
                          const std::vector<int>& field_tfs,
                          const std::vector<int>& field_presence_counts, int field_length,
                          double reference_length, double b_value, int field_count, double bm25_k1,
                          int max_matched) {
    ...
    for (size_t index = 0; index < token_count; ++index) {
        const double idf = std::log1p((1.0 + static_cast<double>(field_count)) /
...
PYBIND11_MODULE(fieldrel, m) {
    m.def("score_field_tokens", &score_field_tokens,
          "Score aligned field tokens where index i refers to the same token in tokens, host_tfs, "
