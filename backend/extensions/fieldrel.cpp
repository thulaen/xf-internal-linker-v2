...
double score_field_tokens(const std::vector<std::string> &tokens,
                          const std::vector<int> &host_tfs,
                          const std::vector<int> &field_tfs,
                          const std::vector<int> &field_presence_counts,
                          int field_length, double reference_length,
                          double b_value, int field_count, double bm25_k1,
                          int max_matched) {
  const size_t token_count = tokens.size();
  if (host_tfs.size() != token_count || field_tfs.size() != token_count ||
      field_presence_counts.size() != token_count) {
    throw std::runtime_error(
        "All token lists must be positionally aligned and the same length");
  }
  ...
  for (size_t index = 0; index < token_count; ++index) {
    const double idf =
        std::log1p((1.0 + static_cast<double>(field_count)) /
                   (1.0 + static_cast<double>(field_presence_counts[index])));
    const double denominator =
        static_cast<double>(field_tfs[index]) +
        bm25_k1 * (1.0 - b_value +
                   b_value * (static_cast<double>(field_length) /
                              std::max(1.0, reference_length)));
    const double tf_norm =
        denominator > 0.0
            ? (static_cast<double>(field_tfs[index]) * (bm25_k1 + 1.0)) /
                  denominator
            : 0.0;
    const double token_score =
        tf_norm * idf * std::min(2.0, static_cast<double>(host_tfs[index]));
  ...
  const size_t keep_count =
      std::min(static_cast<size_t>(max_matched), scored_tokens.size());
...
PYBIND11_MODULE(fieldrel, m) {
  m.def("score_field_tokens", &score_field_tokens,
        "Score aligned field tokens where index i refers to the same token in "
        "tokens, host_tfs, "
        "field_tfs, and field_presence_counts");
}
#endif
