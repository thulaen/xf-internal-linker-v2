...
py::array_t<uint8_t> opq_encode(py::array_t<float> vectors, 
    ...
    if (dim != (size_t)r_info.shape[0] || dim != (size_t)r_info.shape[1]) throw std::runtime_error("Rotation shape mismatch");
    if (dim != m * sub_dim) throw std::runtime_error("Codebook dimension mismatch");
    ...
    c_opq_encode(
        ...
        static_cast<const float*>(cb_info.ptr),
        m, k,
