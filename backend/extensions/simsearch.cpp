...
py::tuple score_and_topk(py::array_t<float> destination,
  ...
  cscore_and_topk(static_cast<const float*>(d_info.ptr), dest_dim,
                  static_cast<const float*>(s_info.ptr), num_sentences,
                  sentence_dim, static_cast<const int32_t*>(c_info.ptr),
                  candidate_count, top_k, out_indices.data(), out_scores.data(),
                  &out_count);
  ...
  std::copy(out_scores.begin(), out_scores.begin() + out_count,
            (float*)res_scores.request().ptr);
