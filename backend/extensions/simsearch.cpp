...
std::tuple<py::array_t<int64_t>, py::array_t<float>>
score_and_topk(py::array_t<float, py::array::c_style | py::array::forcecast>
  ...
  if (destination_buf.ndim != 1) {
    throw std::runtime_error(
        "destination_embedding must be a 1D float32 array");
  }
  ...
  if (sentence_buf.shape[1] != destination_buf.shape[0]) {
    throw std::runtime_error("sentence_embeddings.shape[1] must match "
                             "destination_embedding.shape[0]");
  }
...
#else
      for (size_t index = 0; index < static_cast<size_t>(candidate_count);
           ++index) {
        compute_one(index);
      }
#endif
  ...
  const size_t keep_count = std::min(static_cast<size_t>(top_k),
  ...
  auto compare_positions = [&](int64_t left, int64_t right) {
    return scores[static_cast<size_t>(left)] >
           scores[static_cast<size_t>(right)];
  };
...
PYBIND11_MODULE(simsearch, m) {
  m.def("score_and_topk", &score_and_topk,
        "Score candidate sentence rows and return top-K positional indices "
        "with scores");
}
#endif /* XF_BENCH_MODE */
...
void cscore_and_topk(const float *destination_ptr, size_t dest_dim,
                     ...
                     size_t candidate_count, int top_k, int64_t *out_indices,
