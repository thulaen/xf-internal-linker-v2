...
TEST(CscoreAndTopk, ZeroCandidates) {
  ...
  cscore_and_topk(dest.data(), dim, sentences.data(), 1, dim, nullptr, 0, 5,
...
TEST(CscoreAndTopk, ThreeCandidatesTopTwo) {
  ...
  EXPECT_EQ(out_indices[0],
            1);  // candidate position 1 (row 1, score 0.9) is best
  EXPECT_EQ(out_indices[1],
            2);  // candidate position 2 (row 2, score 0.6) is second
  EXPECT_GT(out_scores[0], out_scores[1]);
  EXPECT_NEAR(out_scores[0], 0.9f, 1e-5f);
  EXPECT_NEAR(out_scores[1], 0.6f, 1e-5f);
}

TEST(CscoreAndTopk, SkipsInvalidRows) {
  const size_t dim = 1;
  std::vector<float> dest = {1.0f};
  std::vector<float> sentences = {0.7f};
  std::vector<int32_t> rows = {-1, 4, 0};
  std::vector<int64_t> out_indices(3);
  std::vector<float> out_scores(3);
  size_t out_count = 0;

  cscore_and_topk(dest.data(), dim, sentences.data(), 1, dim, rows.data(), 3, 3,
                  out_indices.data(), out_scores.data(), &out_count);

  EXPECT_EQ(out_count, 1u);
  EXPECT_EQ(out_indices[0], 2);
  EXPECT_NEAR(out_scores[0], 0.7f, 1e-5f);
}

TEST(CscoreAndTopk, UsesSharedDimensionLimit) {
  std::vector<float> dest = {1.0f, 1.0f, 10.0f};
  std::vector<float> sentences = {0.5f, 0.25f};
  std::vector<int32_t> rows = {0};
  std::vector<int64_t> out_indices(1);
  std::vector<float> out_scores(1);
  size_t out_count = 0;

  cscore_and_topk(dest.data(), 3, sentences.data(), 1, 2, rows.data(), 1, 1,
                  out_indices.data(), out_scores.data(), &out_count);

  EXPECT_EQ(out_count, 1u);
  EXPECT_NEAR(out_scores[0], 0.75f, 1e-5f);
}
