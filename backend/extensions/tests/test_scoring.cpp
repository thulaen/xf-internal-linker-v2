...
TEST(CscoreFullBatch, HandlesMissingSiloAndShortWeights) {
  std::vector<float> components = {0.8f, 0.5f, 0.4f, 0.9f};
  std::vector<float> weights = {0.5f};
  std::vector<float> out(2, 0.0f);

  cscore_full_batch(components.data(), 2, 2, weights.data(), 1, nullptr, 0,
                    out.data());
...
TEST(CscoreFullBatch, IgnoresSiloAfterSiloRowsEnd) {
  std::vector<float> components = {0.8f, 0.5f, 0.4f, 0.9f};
  std::vector<float> weights = {0.5f, 0.5f};
  std::vector<float> silo = {0.1f};
  std::vector<float> out(2, 0.0f);

  cscore_full_batch(components.data(), 2, 2, weights.data(), 2, silo.data(), 1,
                    out.data());

  EXPECT_NEAR(out[0], 0.75f, 1e-5f);
  EXPECT_NEAR(out[1], 0.65f, 1e-5f);
}
