...
def apply_hnsw_indexes(apps, schema_editor):
    if schema_editor.connection.vendor != 'postgresql':
        return
    schema_editor.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS content_item_embedding_hnsw_idx ON content_contentitem USING hnsw (embedding vector_cosine_ops);")
    schema_editor.execute("CREATE INDEX CONCURRENTLY IF NOT EXISTS sentence_embedding_hnsw_idx ON content_sentence USING hnsw (embedding vector_cosine_ops);")
