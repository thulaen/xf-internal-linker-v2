"""Shared settings helpers (Phase 4 + 2026-05-10 extraction).

This module owns three layers of helpers that the per-feature settings
flow (``get_*_settings`` accessors, ``_read_*_settings`` AppSetting
readers, ``_validate_*_settings`` PUT-payload validators) reuses across
the codebase:

1. **Three-tier readers** (``setting_int / _float / _bool / _str``) —
   operator AppSetting → recommended preset → hardcoded fallback. Used
   by services (``passage_relevance``, ``faiss_index``, etc.) that want
   a single import + call.

2. **PUT-payload coercers** (``coerce_setting_*``, ``coerce_clamp_*``,
   ``coerce_lenient_bool``, ``enforce_bounds``, ``_coerce_*_strict``) —
   normalise / validate / clamp raw operator JSON. Strict variants
   raise ``ValueError`` on bad input; clamp variants silently snap to
   the nearest valid value (per spec for FR-013 / FR-023 / FR-025).

3. **Two-tier readers** (``read_app_setting_*``) — operator AppSetting
   → hardcoded fallback (no preset middle layer). Used by the
   per-feature ``_read_*_settings`` functions where the second tier is
   the per-feature default dict, not the recommended preset.

Plus the per-feature ``get_*_settings``, ``_read_*_settings``,
``_validate_*_settings``, and shared ``_get_app_setting_value`` /
``_read_wp_string`` / ``_resolve_wp_app_password`` /
``_ga4_gsc_connection_status`` / ``_sync_wordpress_periodic_task`` that
were previously inlined in ``apps/core/views.py``. Pulling them here on
2026-05-10 dropped ``apps/core/views.py`` below the 1500-line cap and
cleared its grandfather-list entry.

Three-tier semantics is intentional: the recommended-preset middle
layer matters because every Wave-2 ranking signal starts at the spec's
prior weight. ``AppSetting.get_int()`` (Django's existing helper) is
two-tier (operator → fallback) and would lose the preset middle layer,
so we don't just call it directly.
...
# ===========================================================================
# Per-feature DEFAULT_* dicts and shared key/bound tuples.
#
# Moved from apps/core/views.py on 2026-05-10. Each DEFAULT_*_SETTINGS dict
# is the canonical "first start" value for the feature — sourced from the
# recommended-preset module so a Wave-2 weight tweak in one place changes
# every default everywhere.
# ===========================================================================
...
# ===========================================================================
# Per-feature get_*_settings accessors.
# ===========================================================================
...
def get_graph_candidate_settings() -> dict[str, float | int | bool]:
    ...
    # Lazy import: ``_validate_graph_candidate_settings`` lives in
    # ``apps/core/views_capacity.py`` (it is only used by that view layer
    # plus the accessor here). Importing lazily avoids a circular import
    # at module-load time because views_capacity imports from this module.
    from apps.core.views_capacity import _validate_graph_candidate_settings
...
def get_value_model_settings() -> dict[str, float | int | bool]:
    ...
    # Lazy import: ``_validate_value_model_settings`` lives in
    # ``apps/core/views_capacity.py``. Same circular-import reason as
    # ``get_graph_candidate_settings`` above.
    from apps.core.views_capacity import _validate_value_model_settings
...
# ===========================================================================
# Graph-candidate + value-model readers (FR-021 / FR-023 / FR-024 / FR-025).
# ===========================================================================
