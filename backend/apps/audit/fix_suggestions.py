...
_RULES.extend(
    [
        ...
        (
            re.compile(
                r"OPQ.*codebook.*not.*active|opq_codebook_version.*mismatch", re.I
            ),
        ...
        (
            re.compile(
                r"HelperNode.*not.*found|HELPER_NODE_NAME.*not.*registered", re.I
            ),
        ...
        (
            re.compile(
                r"AppSetting.*does not exist|matching query does not exist.*AppSetting",
                re.I,
            ),
        ...
        (
            re.compile(
                r"OPENAI.*api.*key|openai\.AuthenticationError|401.*openai", re.I
            ),
        ...
        (
            re.compile(
                r"GSC.*credentials|google\.auth.*GSC|search-console.*invalid_grant",
                re.I,
            ),
        ...
        (
            re.compile(
                r"GA4.*credentials|GA4.*Unauthorized|analytics\.api_core.*401", re.I
            ),
