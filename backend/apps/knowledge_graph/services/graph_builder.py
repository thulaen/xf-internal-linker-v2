...
def build_knowledge_graph(batch_size=1000):
    ...
    for start in range(0, queryset.count(), batch_size):
        ...
        for item in chunk:
            ...
            token_sets = tokenize_text_batch(
                [text_to_process], STANDARD_ENGLISH_STOPWORDS
            )
            ...
            words = re.findall(
                r"[A-Za-z0-9]+(?:'[A-Za-z0-9]+)?", text_to_process.lower()
            )
            filtered_words = [
                w for w in words if w not in STANDARD_ENGLISH_STOPWORDS and len(w) >= 3
            ]
    ...
    logger.info(
        f"Knowledge graph build complete. Processed: {total_processed}, Skipped: {total_skipped}"
    )
