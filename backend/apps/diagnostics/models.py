...
class ServiceStatusSnapshot(TimestampedModel):
    ...
    service_name = models.CharField(
        max_length=50, choices=SERVICE_CHOICES, db_index=True
    )
    state = models.CharField(
        max_length=30, choices=STATE_CHOICES, default="planned_only"
    )
    ...
    next_action_step = models.TextField(
        blank=True, help_text="User-friendly next step."
    )
    metadata = models.JSONField(
        default=dict, blank=True, help_text="Extra data like version, latency, etc."
    )
...
class SystemConflict(TimestampedModel):
    ...
    location = models.CharField(
        max_length=500, help_text="File path or URL where conflict occurs."
    )
    why = models.TextField(
        blank=True, help_text="Plain-English explanation of the conflict."
    )
