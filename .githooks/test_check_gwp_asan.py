"""Tests for the GWP-ASan AutoIssue commit check."""
...
MODULE_PATH = Path(__file__).with_name("check-gwp-asan.py")
spec = importlib.util.spec_from_file_location("check_gwp_asan", MODULE_PATH)
assert spec and spec.loader
hook = importlib.util.module_from_spec(spec)
spec.loader.exec_module(hook)


class CheckGwpAsanTests(unittest.TestCase):
    def test_passes_when_backend_verifier_passes(self) -> None:
        with patch.object(hook.subprocess, "run") as run:
            run.return_value.returncode = 0
            run.return_value.stdout = "[GWP-ASAN AUTOISSUES VERIFIED: open=0 max=10]"
    ...
    def test_fails_when_backend_verifier_fails(self) -> None:
        with patch.object(hook.subprocess, "run") as run:
            ...
            run.return_value.stderr = "3 open: #1, #2, #3"
    ...
    def test_fail_message_has_three_parts(self) -> None:
        ...
        self.assertIn("FAIL check-gwp-asan", written)
