// Package topicd is the gRPC skeleton for the topicd sidecar service.
...
const ServiceName = "topicd"

// Server is the gRPC handler for topicd.
type Server struct {
	sidecarsv1.UnimplementedTopicdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterTopicdServer(grpcSrv, s)
