// Package ruled is the gRPC skeleton for the ruled sidecar service.
...
const ServiceName = "ruled"

// Server is the gRPC handler for ruled.
type Server struct {
	sidecarsv1.UnimplementedRuledServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterRuledServer(grpcSrv, s)
