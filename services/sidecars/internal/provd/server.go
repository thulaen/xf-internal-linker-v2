// Package provd is the gRPC skeleton for the provd sidecar service.
...
const ServiceName = "provd"

// Server is the gRPC handler for provd.
type Server struct {
	sidecarsv1.UnimplementedProvdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterProvdServer(grpcSrv, s)
