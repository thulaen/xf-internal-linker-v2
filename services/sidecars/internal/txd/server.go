// Package txd is the gRPC skeleton for the txd sidecar service.
...
const ServiceName = "txd"

// Server is the gRPC handler for txd.
type Server struct {
	sidecarsv1.UnimplementedTxdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterTxdServer(grpcSrv, s)
