// Package extractd is the gRPC skeleton for the extractd sidecar service.
...
const ServiceName = "extractd"

// Server is the gRPC handler for extractd.
type Server struct {
	sidecarsv1.UnimplementedExtractdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterExtractdServer(grpcSrv, s)
