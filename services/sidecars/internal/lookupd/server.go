// Package lookupd is the gRPC skeleton for the lookupd sidecar service.
...
const ServiceName = "lookupd"

// Server is the gRPC handler for lookupd.
type Server struct {
	sidecarsv1.UnimplementedLookupdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterLookupdServer(grpcSrv, s)
