// Package timetravd is the gRPC skeleton for the timetravd sidecar service.
...
const ServiceName = "timetravd"

// Server is the gRPC handler for timetravd.
type Server struct {
	sidecarsv1.UnimplementedTimetravdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterTimetravdServer(grpcSrv, s)
