// Package flumed is the gRPC skeleton for the flumed sidecar service.
...
const ServiceName = "flumed"

// Server is the gRPC handler for flumed.
type Server struct {
	sidecarsv1.UnimplementedFlumedServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterFlumedServer(grpcSrv, s)
