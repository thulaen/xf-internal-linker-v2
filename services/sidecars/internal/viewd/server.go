// Package viewd is the gRPC skeleton for the viewd sidecar service.
...
const ServiceName = "viewd"

// Server is the gRPC handler for viewd.
type Server struct {
	sidecarsv1.UnimplementedViewdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterViewdServer(grpcSrv, s)
