// Package cepd is the gRPC skeleton for the cepd sidecar service.
...
const ServiceName = "cepd"

// Server is the gRPC handler for cepd.
type Server struct {
	sidecarsv1.UnimplementedCepdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterCepdServer(grpcSrv, s)
