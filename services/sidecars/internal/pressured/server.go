// Package pressured is the gRPC skeleton for the pressured sidecar service.
...
const ServiceName = "pressured"

// Server is the gRPC handler for pressured.
type Server struct {
	sidecarsv1.UnimplementedPressuredServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterPressuredServer(grpcSrv, s)
