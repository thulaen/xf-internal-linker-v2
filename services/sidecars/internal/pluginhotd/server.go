// Package pluginhotd is the gRPC skeleton for the pluginhotd sidecar service.
...
const ServiceName = "pluginhotd"

// Server is the gRPC handler for pluginhotd.
type Server struct {
	sidecarsv1.UnimplementedPluginhotdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterPluginhotdServer(grpcSrv, s)
	tracker.Register(ServiceName, s, idle.PriorityLow)
	return s
}
