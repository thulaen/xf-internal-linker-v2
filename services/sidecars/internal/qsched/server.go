// Package qsched is the gRPC skeleton for the qsched sidecar service.
...
const ServiceName = "qsched"

// Server is the gRPC handler for qsched.
type Server struct {
	sidecarsv1.UnimplementedQschedServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterQschedServer(grpcSrv, s)
