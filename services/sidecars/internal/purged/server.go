// Package purged is the gRPC skeleton for the purged sidecar service.
...
const ServiceName = "purged"

// Server is the gRPC handler for purged.
type Server struct {
	sidecarsv1.UnimplementedPurgedServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterPurgedServer(grpcSrv, s)
