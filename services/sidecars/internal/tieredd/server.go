// Package tieredd is the gRPC skeleton for the tieredd sidecar service.
...
const ServiceName = "tieredd"

// Server is the gRPC handler for tieredd.
type Server struct {
	sidecarsv1.UnimplementedTiereddServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterTiereddServer(grpcSrv, s)
