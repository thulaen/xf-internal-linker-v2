// Package mesh is the gRPC skeleton for the mesh sidecar service.
...
const ServiceName = "mesh"

// Server is the gRPC handler for mesh.
type Server struct {
	sidecarsv1.UnimplementedMeshServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterMeshServer(grpcSrv, s)
