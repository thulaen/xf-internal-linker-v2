// Package catalogd is the gRPC skeleton for the catalogd sidecar service.
...
const ServiceName = "catalogd"

// Server is the gRPC handler for catalogd.
type Server struct {
	sidecarsv1.UnimplementedCatalogdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterCatalogdServer(grpcSrv, s)
