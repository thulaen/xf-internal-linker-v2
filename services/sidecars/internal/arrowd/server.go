// Package arrowd is the gRPC skeleton for the arrowd sidecar service.
...
const ServiceName = "arrowd"

// Server is the gRPC handler for arrowd.
type Server struct {
	sidecarsv1.UnimplementedArrowdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterArrowdServer(grpcSrv, s)
