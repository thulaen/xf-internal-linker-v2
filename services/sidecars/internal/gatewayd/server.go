// Package gatewayd is the gRPC skeleton for the gatewayd sidecar service.
...
// Server is the gRPC handler for gatewayd.
type Server struct {
	sidecarsv1.UnimplementedGatewaydServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterGatewaydServer(grpcSrv, s)
