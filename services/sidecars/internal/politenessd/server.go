// Package politenessd is the gRPC skeleton for the politenessd sidecar service.
...
const ServiceName = "politenessd"

// Server is the gRPC handler for politenessd.
type Server struct {
	sidecarsv1.UnimplementedPolitenessdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterPolitenessdServer(grpcSrv, s)
