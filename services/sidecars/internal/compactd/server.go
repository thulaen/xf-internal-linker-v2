// Package compactd is the gRPC skeleton for the compactd sidecar service.
...
const ServiceName = "compactd"

// Server is the gRPC handler for compactd.
type Server struct {
	sidecarsv1.UnimplementedCompactdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterCompactdServer(grpcSrv, s)
