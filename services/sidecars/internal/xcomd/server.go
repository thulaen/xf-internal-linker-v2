// Package xcomd is the gRPC skeleton for the xcomd sidecar service.
...
const ServiceName = "xcomd"

// Server is the gRPC handler for xcomd.
type Server struct {
	sidecarsv1.UnimplementedXcomdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterXcomdServer(grpcSrv, s)
