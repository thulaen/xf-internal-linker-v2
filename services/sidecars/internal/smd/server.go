// Package smd is the gRPC skeleton for the smd sidecar service.
...
const ServiceName = "smd"

// Server is the gRPC handler for smd.
type Server struct {
	sidecarsv1.UnimplementedSmdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterSmdServer(grpcSrv, s)
