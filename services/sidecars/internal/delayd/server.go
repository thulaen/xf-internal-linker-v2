// Package delayd is the gRPC skeleton for the delayd sidecar service.
...
const ServiceName = "delayd"

// Server is the gRPC handler for delayd.
type Server struct {
	sidecarsv1.UnimplementedDelaydServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterDelaydServer(grpcSrv, s)
