// Package hintd is the gRPC skeleton for the hintd sidecar service.
...
const ServiceName = "hintd"

// Server is the gRPC handler for hintd.
type Server struct {
	sidecarsv1.UnimplementedHintdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterHintdServer(grpcSrv, s)
