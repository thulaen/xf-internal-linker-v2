// Package tsd is the gRPC skeleton for the tsd sidecar service.
...
const ServiceName = "tsd"

// Server is the gRPC handler for tsd.
type Server struct {
	sidecarsv1.UnimplementedTsdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterTsdServer(grpcSrv, s)
