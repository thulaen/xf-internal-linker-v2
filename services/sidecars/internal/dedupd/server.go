// Package dedupd is the gRPC skeleton for the dedupd sidecar service.
...
const ServiceName = "dedupd"

// Server is the gRPC handler for dedupd.
type Server struct {
	sidecarsv1.UnimplementedDedupdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterDedupdServer(grpcSrv, s)
