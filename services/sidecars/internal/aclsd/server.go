// Package aclsd is the gRPC skeleton for the aclsd sidecar service.
...
const ServiceName = "aclsd"

// Server is the gRPC handler for aclsd.
type Server struct {
	sidecarsv1.UnimplementedAclsdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterAclsdServer(grpcSrv, s)
