// Package fnd is the gRPC skeleton for the fnd sidecar service.
...
const ServiceName = "fnd"

// Server is the gRPC handler for fnd.
type Server struct {
	sidecarsv1.UnimplementedFndServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterFndServer(grpcSrv, s)
