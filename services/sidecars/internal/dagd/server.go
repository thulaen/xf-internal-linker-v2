// Package dagd is the gRPC skeleton for the dagd sidecar service.
...
const ServiceName = "dagd"

// Server is the gRPC handler for dagd.
type Server struct {
	sidecarsv1.UnimplementedDagdServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterDagdServer(grpcSrv, s)
