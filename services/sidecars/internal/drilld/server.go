// Package drilld is the gRPC skeleton for the drilld sidecar service.
...
const ServiceName = "drilld"

// Server is the gRPC handler for drilld.
type Server struct {
	sidecarsv1.UnimplementedDrilldServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterDrilldServer(grpcSrv, s)
