// Package gremlind is the gRPC skeleton for the gremlind sidecar service.
...
const ServiceName = "gremlind"

// Server is the gRPC handler for gremlind.
type Server struct {
	sidecarsv1.UnimplementedGremlindServer
	store     *bbolt.DB
	logger    *slog.Logger
	startedAt time.Time
}
...
func Register(grpcSrv *grpc.Server, db *bbolt.DB, tracker *idle.Tracker, logger *slog.Logger) *Server {
	...
	sidecarsv1.RegisterGremlindServer(grpcSrv, s)
