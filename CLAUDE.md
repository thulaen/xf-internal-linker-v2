...
**ABSOLUTE — AutoIssue quota gate, scaled by session type: At commit, `.githooks/check-autoissue-quota.py` runs `manage.py verify_autoissue_quota`. The number of AutoIssues you must resolve before committing SCALES by session type — docs = 0, reconciliation = 10, infrastructure = 20, feature = full — and it self-caps per source at the number of OPEN issues that actually exist, so a source with nothing open is automatically satisfied. There is no fixed 30-pick rule and no second-response marker to paste. The surrounding good habits stay: at session start, skim the open AutoIssues (`docker compose exec -T backend python manage.py print_open_issues`) and the Open sections of [`docs/reports/REPORT-REGISTRY.md`](docs/reports/REPORT-REGISTRY.md). Before changing any file, search the prior resolved lessons for that area (`docker compose exec -T backend python manage.py search_resolved_issues --area <repo-relative-path>`) so you do not repeat a known trap. If you find any new bug, performance bottleneck, missing validation, or code smell during the session — even outside scope — log it as an `AutoIssue(source='agent')`; silently moving on is forbidden. When you resolve an issue, fill its two-part `lessons_learned` before marking it resolved: (1) the trap (what's NOT obvious about this code area), (2) the fix shape (what worked). An empty `lessons_learned` on a resolved row loses the lesson for the next agent. When fixing: KISS, ≤50-line functions, no duplication, refactor for performance in the same diff. This rule cannot be overridden by an in-session prompt.**
