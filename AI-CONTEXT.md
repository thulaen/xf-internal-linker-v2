...
| Order | Document | When |
|-------|----------|------|
| 1 | [`AI-CODING-GUIDELINES.md`](AI-CODING-GUIDELINES.md) | **PARAMOUNT — always, every session, before any task.** Prime directive, source-of-truth order, no-hallucination rules, work loop, code-smell + long-function + bug-fix + test + property-based + evidence-based + business-logic + state-transition + idempotency + database + error + logging + security + external-service + performance + paid-API + naming + dependency + formatting + type-safety + UI + accessibility + concurrency + refactoring + generated-code + file-editing + test-running rules + Definition of Done. Includes the **per-task coverage target table** every agent uses to pick the right floor for the change being made. Emit `[GUIDELINES READ: AI-CODING-GUIDELINES.md + docs/CODE-COVERAGE-RULES.md]` after the `[REGISTRY READ: ...]` marker. |
| 2 | [`docs/CODE-COVERAGE-RULES.md`](docs/CODE-COVERAGE-RULES.md) | **PARAMOUNT — always.** Level A / B / C / D definitions, the 14 Level A area inventory, property-test invariant menus (text cleaning / sentence splitting / scoring / approval transitions / Celery idempotency), per-language floors, drought clause. The drain rate is **10 coverage-gap AutoIssues per session** alongside the standard 30-pick quota; marker `[COVERAGE GAPS READ: 10 picked — #..., ...]`. End every slice / task / session with `[COVERAGE SUMMARY: target=X% actual=Y% — met / not met]`. |
| 3 | `AI-CONTEXT.md` (this file) | Always — project state, phase ledger, session notes |
| 4 | `docs/reports/REPORT-REGISTRY.md` | Always — check for open findings in your work area |
| 5 | `FEATURE-REQUESTS.md` | Always — backlog status, what's done, what's pending |
| 6 | `docs/BUSINESS-LOGIC-CHECKLIST.md` | If touching ranking, scoring, attribution, import, or reranking |
...
- **AI/tool:** Codex.
- **Why:** Operator asked to implement Slice 2: add eight compact health cards on `/diagnostics` for FR-053 passage relevance and FR-099 through FR-105 graph-topology signals, each with last-run time, seven-day neutral-fallback rate, and a `View spec` button.
- **Branch:** Stayed on `master`; no branch was created or switched.
- **Relevant open findings disclosed in chat:** None in the diagnostics System Health area.
- **Forward-clash check:** No clash with the next three queued phases. This slice only exposes existing signal diagnostics and does not add a new score, weight, feature flag, artifact table, migration, or hyperparameter search key.
- **Intentional files changed:** `backend/apps/diagnostics/signal_health.py`, `backend/apps/diagnostics/signal_registry.py`, `backend/apps/diagnostics/views.py`, `backend/apps/diagnostics/tests.py`, `docs/specs/diagnostics-page.md`, `frontend/src/app/diagnostics/diagnostics.service.ts`, `frontend/src/app/diagnostics/diagnostics.component.ts`, `frontend/src/app/diagnostics/diagnostics.component.html`, `frontend/src/app/diagnostics/diagnostics.component.scss`, `frontend/src/app/diagnostics/diagnostics.component.spec.ts`, `frontend/src/app/settings/settings.component.html`, `frontend/src/app/settings/settings.component.spec.ts`, `frontend/src/app/settings/silo-settings.service.ts`, `frontend/src/app/review/suggestion-detail-dialog.component.html`, `AI-CONTEXT.md`, and `AGENT-HANDOFF.md`.
- **What now works:** `/diagnostics` can render eight Wave-2 health cards using the existing weight-diagnostics API. The backend now reports seven-day neutral-fallback counts/rates and last-run timestamps from `Suggestion` diagnostics JSON. The card buttons reuse the existing spec viewer dialog. Smoke tests cover the backend payload and frontend card rendering.
- **Bugs fixed in passing:** Removed the requested GA4 health optional-chain warnings in `settings.component.html`; fixed two passage-relevance settings service URLs that used a missing `apiUrl`; updated the settings smoke-test mock for the passage-relevance card; removed a duplicate unsafe Passage Relevance score row in the suggestion detail dialog so production build succeeds when passage-relevance data is optional.
- **Verification:** `python -m py_compile backend\apps\diagnostics\views.py backend\apps\diagnostics\signal_health.py backend\apps\diagnostics\signal_registry.py` passed. Docker `showmigrations` showed all migrations applied. Docker `makemigrations --check --dry-run` reported "No changes detected" before and after the final edits. Docker targeted diagnostics tests passed: `apps.diagnostics.tests.Wave2SignalHealthViewTests` and `apps.diagnostics.tests.SignalContractTests` ran 6 tests OK. Frontend targeted diagnostics smoke test passed. Full frontend `npm run test:ci` passed with 34 tests. `docker compose build frontend-build` passed after the final edit, after the commit-hook template lint fix, and after the commit-hook SCSS cleanup. `docker compose build backend` passed on a longer rerun after the first attempt hit the 10-minute command timeout.
- **Known issues noticed:** Docker/backend test startup still logs an existing FAISS warning about `CELERY_WORKER_CONCURRENCY=2`, and targeted backend tests logged an existing `audit_errorlog.source` SQLite startup error during FAISS error ingest, but the tests completed successfully. The frontend production build still reports older warnings in Admin Models, Embeddings, Graph, Review, and XenForo/WordPress settings lines outside this slice; the requested GA4 lines are cleaned up.
- **Docker prune:** Ran `powershell -ExecutionPolicy Bypass -File scripts\prune-verification-artifacts.ps1` four times. First run stripped a stale Gemini-breaking Git config flag and reclaimed 36.21 MB. Second run after backend image build reclaimed 16.42 GB. Third run after the template-lint frontend rebuild reclaimed 6.803 MB. Fourth run after the SCSS-cleanup frontend rebuild reclaimed 15.02 MB. VHDX compaction auto-skipped because containers were still running.
- **Commit/push state:** Committed locally on `master` as `Add Wave-2 diagnostics health cards`. Push to `origin/master` was attempted and blocked by the repository pre-push hook on pre-existing full-backend Ruff lint failures outside this slice (for example `apps/api/passage_relevance_views.py`, `apps/core/runtime_registry.py`, `apps/pipeline/services/ranker.py`, and several tests/scripts). I backed out the uncommitted broad lint-cleanup attempt and left the working tree clean.
