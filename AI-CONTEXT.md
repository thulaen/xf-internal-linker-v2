...
- Session target: embedding model infrastructure upgrade only
- What changed:
  - `DEFAULT_MODEL_NAME` now falls back to `nomic-ai/nomic-embed-text-v1.5`
  - `EMBEDDING_DIM` now uses 768
  - `trust_remote_code=True` is now set when loading the embedding model
  - `search_document:` is now added in both embedding encode paths
  - both pgvector `VectorField` dimensions now use 768
  - the pipeline fallback embedding-dimension constant now uses 768
  - a new content migration was created for the 768-dimension field change
- What was intentionally not changed:
  - no FR phase work
  - no tests
  - no frontend
  - no ranking logic
  - no performance-mode architecture
  - no DB override architecture for `AppSetting("embedding_model")`
- Operator note:
  - if `AppSetting("embedding_model")` already exists and still points to the old model, the operator must update that setting or the old model may still be used
- Operator note:
  - after deploying this migration, old stored 384-dimension embeddings are stale and must be regenerated by running `generate_all_embeddings()`
- Verification completed:
  - migration drift check passed
  - code checks passed
  - both performance modes preserved
