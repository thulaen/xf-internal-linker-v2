...
| 2 | [`docs/CODE-COVERAGE-RULES.md`](docs/CODE-COVERAGE-RULES.md) | **PARAMOUNT — always.** Level A / B / C / D definitions, the 14 Level A area inventory, property-test invariant menus (text cleaning / sentence splitting / scoring / approval transitions / Celery idempotency), per-language floors, drought clause. The standard 30-pick AutoIssue quota still applies. End every slice / task / session with `[COVERAGE SUMMARY: target=X% actual=Y% — met / not met]`. |
