...
| 2 | [`docs/CODE-COVERAGE-RULES.md`](docs/CODE-COVERAGE-RULES.md) | **PARAMOUNT — always.** Level A / B / C / D definitions, the 14 Level A area inventory, property-test invariant menus (text cleaning / sentence splitting / scoring / approval transitions / Celery idempotency), per-language floors, drought clause. The drain rate is **10 coverage-gap AutoIssues per session** alongside the standard 18-pick quota; marker `[COVERAGE GAPS READ: 10 picked — #..., ...]`. End every slice / task / session with `[COVERAGE SUMMARY: target=X% actual=Y% — met / not met]`. |
