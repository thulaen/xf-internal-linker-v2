...
| the project's session-start rule raised to 30 picks (3 per source × 10 sources) on 2026-05-12 — every AI agent must fix three open issues from EACH of the ten sources (agent, glitchtip, pyroscope, tempo, loki, faro, mutation, fuzz, contract, gh_ci) before working on whatever the user asked for; the satisfier phrase `auto-fix-30 satisfier` replaces the picks half when the session task itself is a multi-bug fix. Legacy phrases `auto-fix-18 satisfier` / `auto-fix-12 satisfier` / `auto-fix-3 satisfier` remain accepted for backwards compat with historical handoff entries on master | auto-fix-30 / auto-fix-30 satisfier / fix thirty before any new task |
