...
@Component({
  ...
  styles: [
    `
      ...
      .ih-good {
        background: var(--color-success-light, rgba(30, 142, 62, 0.06));
      }
      .ih-warn {
        background: var(--color-warning-light, rgba(249, 171, 0, 0.1));
      }
      .ih-bad {
...
export class InstantHealthComponent {
  ...
  readonly verdictSubtitle = computed<string>(() => {
    switch (this.verdict()) {
      case 'good':
        return 'Everything is healthy right now.';
      case 'warn':
        return 'Some services degraded — worth a glance.';
      case 'bad':
        return 'A critical issue is active. Open Health.';
    }
  });
}
