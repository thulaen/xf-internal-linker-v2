...
export interface ResumeState {
  interrupted_runs: { run_id: string; run_state: string; created_at?: string; updated_at?: string }[];
  resumable_syncs: {
    job_id: string;
    status: string;
    source: string;
  ...
  missed_tasks: { task_name: string; weight_class: string; hours_overdue: number | null; reason: string }[];
}

@Component({
  selector: 'app-pick-up',
  standalone: true,
  imports: [CommonModule, MatCardModule, MatIconModule, MatButtonModule, EmptyStateComponent],
        ...
        @if (isEmpty) {
          ...
          @for (run of resumeState.interrupted_runs; track run.run_id) {
            <div class="resume-row">
              ...
              <span class="row-label">Pipeline {{ shortId(run.run_id) }}: {{ run.run_state }}</span>
          ...
          @for (task of resumeState.missed_tasks; track task.task_name) {
            <div class="resume-row">
              ...
              <span class="row-label">{{ task.task_name }}: {{ task.reason }}</span>
          ...
          @for (sync of resumeState.resumable_syncs; track sync.job_id) {
            <div class="resume-row">
              ...
              <span class="row-label">
                {{ sync.source | uppercase }} {{ sync.mode }}: {{ sync.checkpoint_stage }} after {{ sync.checkpoint_items_processed | number }} items
              </span>
