...
@Component({
      ...
      <mat-card-content>
        ...
        @if (currentMode === 'high') {
          <div
            class="expiry-row"
            role="radiogroup"
            aria-label="Auto-revert timer"
            i18n-aria-label="@@performanceMode.autoRevertTimerLabel"
          >
      ...
      <mat-accordion class="help-accordion">
        <mat-expansion-panel class="help-panel">
          ...
          <dl class="glossary">
            ...
            <dd i18n="@@performanceMode.safeDesc">
              Uses conservative CPU and memory limits. You can keep browsing and working while the linker runs quietly
              in the background.
            </dd>

            <dt i18n="@@performanceMode.balancedLabel">Balanced (default)</dt>
            <dd i18n="@@performanceMode.balancedDesc">
              Good speed without hogging your computer. Mostly uses the CPU; a smart choice most of the time.
            </dd>

            <dt i18n="@@performanceMode.highLabel">High Performance</dt>
            <dd i18n="@@performanceMode.highDesc">
              Goes full throttle on CPU-backed work. Close Chrome tabs before switching.
            </dd>

            <dt i18n="@@performanceMode.batchSizeLabel">Batch size</dt>
            <dd i18n="@@performanceMode.batchSizeDesc">
              How many paragraphs the linker processes at the same time. Bigger batch = faster, but uses more memory.
              Adjustable in Settings → Performance.
            </dd>

            <dt i18n="@@performanceMode.workerLabel">Worker</dt>
            <dd i18n="@@performanceMode.workerDesc">
              A helper process that runs background jobs (imports, scoring, etc). More workers = more things in
              parallel, but also more memory used. Changes need a restart to apply.
            </dd>
          </dl>
        </mat-expansion-panel>
      </mat-accordion>
  ...
  styles: [
    `
      ...
      .mode-button.unavailable .mode-label {
...
export class PerformanceModeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http
      .get<{ armed: boolean }>('/api/system/safe-mode-boot/')
      .pipe(
        catchError(() => EMPTY),
    ...
    // across tabs.
    this.perfMode.refresh().pipe(takeUntilDestroyed(this.destroyRef)).subscribe();
  }
  ...
  setExpiry(next: PerformanceExpiry): void {
    ...
    this.perfMode
      .setExpiry(next, expiresAt)
      .pipe(
        catchError(() => {
          this.snack.open(
            $localize`@@performanceMode.saveExpiryError:Could not save revert timer. Try again.`,
  ...
  armSafeModeBoot(): void {
    this.http
      .post<{ armed: boolean }>('/api/system/safe-mode-boot/', {})
      .pipe(
        catchError(() => {
  ...
  private applyMode(key: string): void {
    ...
    this.http
      .post<void>('/api/settings/runtime/switch/', { mode: key })
      .pipe(
        catchError(() => {
      ...
      .subscribe(() => {
        ...
        if (opt) {
          ...
          this.snack.open(
            $localize`@@performanceMode.switchedTo:Switched to ${label}:label:`,
            $localize`@@common.ok:OK`,
            { duration: 2500 },
          );
        }
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <p i18n="@@confirmHighPerformance.memoryWarning">This mode raises CPU and memory limits for local work.</p>
      <p i18n="@@confirmHighPerformance.performanceWarning">
        If your browser or other apps need the same CPU and RAM, they may slow down or stutter. It is best to close
        heavy apps before switching.
      </p>
    ...
    <mat-dialog-actions align="end">
      ...
      <button
        ...
        i18n="@@confirmHighPerformance.yesSwitch"
      >
        Yes, switch
      </button>
    </mat-dialog-actions>
