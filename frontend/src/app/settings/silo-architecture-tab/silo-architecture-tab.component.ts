...
import { ScopeItem, SiloGroup, SiloMode, SiloSettings, SiloSettingsService } from '../silo-settings.service';
...
export class SiloArchitectureTabComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.siloSvc
      ...
      .subscribe({
        ...
        error: () => {
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.loadSettingsFailed:Failed to load silo settings`,
  ...
  private loadGroupsAndScopes(): void {
    this.siloSvc
      ...
      .subscribe({
        next: (groups) => {
          ...
          this.siloSvc
            ...
            .subscribe({
              ...
              error: () => {
                this.snack.open(
                  $localize`:@@settings.siloArchitecture.errors.loadScopesFailed:Failed to load scopes`,
                  $localize`:@@settings.actions.dismiss:Dismiss`,
                  { duration: 4000 },
                );
              },
        ...
        error: () => {
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.loadGroupsFailed:Failed to load silo groups`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
    ...
    this.siloSvc
      ...
      .subscribe({
        next: (settings) => {
          ...
          this.snack.open($localize`:@@settings.siloArchitecture.success.saved:Silo settings saved`, undefined, {
            duration: 2500,
          });
        },
        error: (error) => {
          ...
          this.snack.open(
            error?.error?.detail ||
              $localize`:@@settings.siloArchitecture.errors.saveFailed:Failed to save silo settings`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
  ...
  createGroup(): void {
    if (!this.newGroup.name.trim()) {
      this.snack.open(
        $localize`:@@settings.siloArchitecture.errors.nameRequired:Group name is required`,
        $localize`:@@settings.actions.dismiss:Dismiss`,
        { duration: 3000 },
      );
      return;
    ...
    this.siloSvc
      ...
      .subscribe({
        next: (group) => {
          ...
          this.snack.open($localize`:@@settings.siloArchitecture.success.groupCreated:Silo group created`, undefined, {
            duration: 2500,
          });
        },
        error: () => {
          this.creatingGroup.set(false);
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.groupCreateFailed:Failed to create silo group`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
    ...
    this.siloSvc
      ...
      .subscribe({
        next: (updated) => {
          ...
          this.snack.open($localize`:@@settings.siloArchitecture.success.groupUpdated:Silo group updated`, undefined, {
            duration: 2500,
          });
        },
        error: () => {
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.groupUpdateFailed:Failed to update silo group`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
  ...
  deleteGroup(group: SiloGroup): void {
    this.siloSvc
      .deleteSiloGroup(group.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.siloGroups = this.siloGroups.filter((item) => item.id !== group.id);
          this.scopes = this.scopes.map((scope) =>
            scope.silo_group === group.id ? { ...scope, silo_group: null, silo_group_name: '' } : scope,
          );
          this.snack.open($localize`:@@settings.siloArchitecture.success.groupDeleted:Silo group deleted`, undefined, {
            duration: 2500,
          });
        },
        error: () => {
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.groupDeleteFailed:Failed to delete silo group`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
  ...
  updateScope(scope: ScopeItem, siloGroupId: number | null): void {
    this.siloSvc
      .updateScopeSilo(scope.id, siloGroupId)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (updated) => {
          Object.assign(scope, updated);
          this.snack.open(
            $localize`:@@settings.siloArchitecture.success.scopeSaved:Scope assignment saved`,
            undefined,
            { duration: 2000 },
          );
        },
        error: () => {
          this.snack.open(
            $localize`:@@settings.siloArchitecture.errors.scopeSaveFailed:Failed to save scope assignment`,
            $localize`:@@settings.actions.dismiss:Dismiss`,
            { duration: 4000 },
          );
        },
