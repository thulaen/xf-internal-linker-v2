...
@Component({
  ...
  template: `
    @if (open()) {
      <div class="gd-backdrop" (click)="onClose()" aria-hidden="true"></div>
      <aside class="gd-panel" role="dialog" aria-labelledby="gd-title" aria-modal="true">
        <header class="gd-header">
          <h2 id="gd-title" class="gd-title">
            <mat-icon class="gd-title-icon">menu_book</mat-icon>
            Glossary
          </h2>
          <button mat-icon-button type="button" class="gd-close" aria-label="Close glossary" (click)="onClose()">
        ...
        <div class="gd-list" role="list">
          @if (filtered().length === 0) {
            <p class="gd-empty">No matches. Try a shorter query, or ask in chat — we may need to add this term.</p>
          } @else {
        ...
        <footer class="gd-footer">
          ...
          <span class="gd-shortcut"> Press <kbd>Alt</kbd> + <kbd>G</kbd> to toggle this drawer </span>
        </footer>
      </aside>
    }
  `,
...
export class GlossaryDrawerComponent {
  ...
  readonly filtered = computed<readonly GlossaryEntry[]>(() => {
    ...
    return sorted.filter((e) => e.term.toLowerCase().includes(q) || e.definition.toLowerCase().includes(q));
  });
