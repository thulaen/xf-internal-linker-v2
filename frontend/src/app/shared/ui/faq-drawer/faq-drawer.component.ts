...
@Component({
  ...
  imports: [FormsModule, MatIconModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatExpansionModule],
  template: `
    @if (open()) {
      <div class="fd-backdrop" (click)="onClose()" aria-hidden="true"></div>
      <aside class="fd-panel" role="dialog" aria-labelledby="fd-title" aria-modal="true">
        <header class="fd-head">
          <h2 id="fd-title" class="fd-title">
            <mat-icon class="fd-title-icon">contact_support</mat-icon>
            FAQ
          </h2>
          <button mat-icon-button type="button" aria-label="Close FAQ" (click)="onClose()">
        ...
        <div class="fd-list">
          @if (filtered().length === 0) {
            <p class="fd-empty">Nothing matched. Try the help chatbot (bottom-right) or open the glossary (Alt + G).</p>
          } @else {
            <mat-accordion>
...
export class FaqDrawerComponent {
  ...
  onQueryChange(next: string): void {
    ...
    this.filtered.set(FAQ.filter((e) => e.question.toLowerCase().includes(q) || e.answer.toLowerCase().includes(q)));
  }
}
