...
**ABSOLUTE — Report-only Agent Guard checker enforces TDD, KISS, and DRY: (added 2026-05-27)** The `scripts/agent_guard.py` post-invocation checker verifies coding rules. If an agent attempts to save a production file without first modifying a test file (TDD Guard), writes a function > 50 lines or > 10 complexity (KISS Guard), or copy-pastes a 6+ line block of code (DRY Guard), the checker will print a CAUTION message and log a deduped finding to the AutoIssues database. It runs manually via `python scripts/agent_guard.py` and automatically at commit time via pre-commit soft gate. It will never revert or modify your files. Follow the rules exactly: write tests first, keep functions small, and refactor for reuse.
...
**ABSOLUTE — Python quality/security/lint/mutation/coverage/test tools run in `backend-quality`, never the runtime `backend`, never the host.** The lean `backend` (runtime) image deliberately omits `ruff`, `pylint`, `bandit`, `mutmut`, `coverage`, `pytest`, `safety`, and `pip-audit` — they live ONLY in the `backend-quality` image (the Dockerfile `quality` stage). Compiled-language quality tools live in `compiled-tools` (on the Mint helper). So:
- Run a Python quality/security/lint/mutation/coverage/pytest tool with `docker compose run --rm backend-quality <tool> ...` (or the quality-runner scripts). `docker compose exec backend <tool>` WILL fail — the tool is not there — and you MUST NOT fall back to a host `pip install` / `pipx install` (host-only tools are forbidden).
- Node/JS quality tools (`eslint`, `stylelint`, `stryker`) run in `frontend-mutation-tools`; never `npm install -g` on the host.
- Compiled-language tools (`mutmut` for C++/`mull`, `clang-tidy`, `go-mutesting`, etc.) run via `docker --context mint exec xf_linker_compiled_tools <tool> ...`.
