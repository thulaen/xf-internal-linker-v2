...
Tests that mypy type-checking is wired alongside ruff, pylint, and bandit.
"""
...
def test_mypy_is_wired():
    ...
    text = _text()
    assert "mypy" in text, (
        "run-python-quality.sh must call python -m mypy so Python type errors "
        "are caught at the same stage as ruff and pylint"
    )
...
def test_mypy_is_a_quality_step():
    """mypy must be logged through run_quality_step.py like ruff and pylint."""
    text = _text()
