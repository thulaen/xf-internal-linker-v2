...
$output = docker --context dell run --rm -v xf_dell_owasp_reports:/reports alpine sh -c "ls -l /reports/dependency-check-report.html 2>/dev/null"

if ($output -match "dependency-check-report.html") {
    $date = docker --context dell run --rm -v xf_dell_owasp_reports:/reports alpine stat -c %y /reports/dependency-check-report.html
