...
def _targets_for_path(path: str) -> set[str]:
    ...
    if path.startswith(("backend/", "scripts/")) and path.endswith(".py"):
        targets.add("//tools/quality:python")
    if path.startswith("frontend/"):
        targets.add("//tools/quality:frontend")
    if path.startswith(("rust/", "services/speccheck/")) or path in {
        "Cargo.toml",
        "Cargo.lock",
        "rust-toolchain.toml",
    }:
        targets.add("//tools/quality:rust")
...
def changed_paths() -> list[str]:
    ...
    result = subprocess.run(
        [sys.executable, "scripts/commit_scope.py", "paths", "--mode", "worktree"],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\n".join(targets or [DEFAULT_TARGET]))
    return 0
