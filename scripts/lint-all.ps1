...
$baselineLongFiles = @(
    ...
    # ── Added 2026-05-09 (catch-up push after a backend ruff format pass)
    # — pre-existing 80-line-cap violations surfaced when formatter line-
    # numbering pushed many functions over the diff-scoped detector. None
    # were introduced by the catch-up sessions; each is a candidate for
    # a future refactor pass.
    'passage_relevance.py',              # pipeline/services/passage_relevance.py — 3 long ranking helpers.
    'candidate_retrievers.py',           # pipeline/services/candidate_retrievers.py — 2 retriever entry points.
    'phrase_matching.py',                # pipeline/services/phrase_matching.py — 2 long phrase-score helpers.
    'rare_term_propagation.py',          # pipeline/services/rare_term_propagation.py — 2 long propagation helpers.
    'apps.py',                           # pipeline/apps.py — single long ready() with FAISS/single-worker probe + monthly schedule registration.
    'tasks_gpu_cleanup.py',              # core/tasks_gpu_cleanup.py — single long cleanup task.
    'slowness_analyzer.py',              # diagnostics/services/slowness_analyzer.py — single long analyser entry point.
    'graph_sync.py',                     # graph/services/graph_sync.py — single long sync entry point.
    'graph_builder.py',                  # knowledge_graph/services/graph_builder.py — single long graph build.
    'faiss_index.py',                    # pipeline/services/faiss_index.py — single long index-build helper.
    'field_aware_relevance.py',          # pipeline/services/field_aware_relevance.py — single long evaluator.
    'learned_anchor.py',                 # pipeline/services/learned_anchor.py — single long evaluator.
    'link_freshness.py',                 # pipeline/services/link_freshness.py — single long evaluator.
    'nlp_enrichment.py',                 # pipeline/services/nlp_enrichment.py — single long enrichment pass.
    'slate_diversity.py',                # pipeline/services/slate_diversity.py — single long MMR-style helper.
    'weighted_pagerank.py',              # pipeline/services/weighted_pagerank.py — single long compute helper.
    '0054_upsert_meta_expansion_slots.py', # suggestions migration — long data-migration body (one-shot).
    'weight_tuner.py',                   # suggestions/services/weight_tuner.py — long L-BFGS run() body; candidate for per-stage extraction.
    # ── File-length cap entries (rule #15) — same 2026-05-09 catch-up
    # push, files exceeding the 500/400-line cap that aren't tests or
    # migrations. Each is pre-existing tech debt with a candidate refactor.
    'preferences.component.ts',          # frontend preferences page — 870 lines, 9 sections inline; candidate for per-section component extraction.
    'suggestion.service.ts',             # frontend suggestion service — 648 lines, all suggestion-related HTTP wrappers; candidate for sub-service split.
    'check-forbidden-patterns.py',       # .githooks/check-forbidden-patterns.py — 596 lines, ~30 regex rules inline; candidate for per-rule helper extraction.
    'diagnostics.component.ts',          # frontend diagnostics page — 566 lines, error-log + nodes + runtime + tabs in one component; candidate for per-section split.
    'recommended_weights_forward_249.py', # suggestions/recommended_weights_forward_249.py — 529 lines of forward-declared FR-249 defaults; data file by design.
    'dev_tools_checks.py',               # 507 lines of dev-tools health probes; candidate for per-probe extraction.
...
# Exclude services that intentionally delegate error handling to the calling component
$serviceTsFiles = @($serviceTsFiles | Where-Object { $_ -like "*.service.ts" -and $_ -notmatch 'suggestion\.service|dashboard\.service|sync\.service|silo-settings\.service|behavioral-hub\.service|health\.service|crawler\.service|runtime-models\.service|auth\.service|passkey\.service|meta-algorithms\.service' })
