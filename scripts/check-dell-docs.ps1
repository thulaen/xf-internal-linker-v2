...
$inspect = ssh dell docker inspect -f '{{.Name}} {{.State.Running}} {{.RestartCount}}' xf_dell_docs
...
$healthResponse = ssh dell docker run --rm --network xf_dell_quality alpine wget -qO- http://xf_dell_docs:80
if ($LASTEXITCODE -ne 0) {
